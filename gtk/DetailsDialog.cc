...
void DetailsDialog::Impl::setPeerViewColumns()
{
    ...
    bool const more = settings_->get_boolean("show-extra-peer-details");
...
void DetailsDialog::Impl::peer_page_init(Glib::RefPtr<Gtk::Builder> const& builder)
{
    ...
    settings_->bind("show-extra-peer-details", more_peer_details_check_->property_active());
    settings_->signal_changed("show-extra-peer-details")
...
{
    /* show all */
    if (settings_->get_boolean("show-backup-trackers"))
    {
        return true;
    }
...
void DetailsDialog::Impl::refreshTracker(std::vector<tr_torrent*> const& torrents)
{
    ...
    bool const showScrape = settings_->get_boolean("show-tracker-scrapes");
...
void DetailsDialog::Impl::tracker_page_init(Glib::RefPtr<Gtk::Builder> const& /*builder*/)
{
    ...
    settings_->bind("show-tracker-scrapes", scrape_check_->property_active());
    settings_->signal_changed("show-tracker-scrapes").connect(sigc::hide<0>(sigc::mem_fun(*this, &Impl::refresh)));

    settings_->bind("show-backup-trackers", all_check_->property_active());
    settings_->signal_changed("show-backup-trackers").connect(sigc::hide<0>(sigc::mem_fun(*this, &Impl::refresh)));
}
    ...
    , file_label_(gtr_get_widget<Gtk::Label>(builder, "files_label"))
{
    ...
    gtr_bind_window_state_settings(dialog, settings, "details-window");
