...
void Application::Impl::on_startup()
{
    ...
    if (gtr_pref_flag_get(TR_KEY_blocklist_enabled) && settings_->get_boolean("blocklist-updates-enabled"))
    {
...
void Application::Impl::app_setup()
{
    if (is_iconified_)
    {
        settings_->set_boolean("show-notification-area-icon", true);
    }
    ...
    /* set up the icon */
    settings_->signal_changed("show-notification-area-icon")
    ...
    if (!settings_->get_boolean("user-has-given-informed-consent"))
    {
        ...
        w->signal_response().connect(
            [this, w](int response) mutable
            {
                ...
                {
                    // only show it once
                    settings_->set_boolean("user-has-given-informed-consent", true);
                    w.reset();
                }
                else
                {
...
{
    if (bool const show = settings_->get_boolean("show-notification-area-icon"); show && icon_ == nullptr)
    {
