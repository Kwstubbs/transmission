...
void OptionsDialog::Impl::addResponseCB(int response)
{
    if (tor_ != nullptr)
    {
        if (response == TR_GTK_RESPONSE_TYPE(ACCEPT))
        {
            ...
            {
                gtr_file_trash_or_remove(filename_, settings_->get_boolean("trash-can-enabled"), nullptr);
            }
    ...
    : Gtk::FileChooserDialog(parent, _("Open a Torrent"), TR_GTK_FILE_CHOOSER_ACTION(OPEN))
{
    ...
    auto last_dir = Glib::locale_from_utf8(settings->get_string("open-dialog-dir"));
    if (last_dir.empty())
    {
    ...
    set_choice(ShowOptionsDialogChoice, settings->get_boolean("show-options-window") ? "true" : "false");
}
